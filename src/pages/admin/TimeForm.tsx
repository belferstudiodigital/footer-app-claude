...
interface FormState {
  ...
  aviso_semanal_ativo: boolean
  dia_aviso_semanal: number
  horario_aviso_semanal: string
...
const PADRAO: FormState = {
  ...
  aviso_semanal_ativo: false,
  dia_aviso_semanal: 3,
  horario_aviso_semanal: '09:00',
  whatsapp_admin: '',
  valor_avulso: '',
  chave_pix: '',
  recados: '',
    ...
    ;(async () => {
      ...
      if (data) {
        setForm({
          ...
          aviso_semanal_ativo: data.dia_aviso_semanal != null && data.horario_aviso_semanal != null,
          dia_aviso_semanal: data.dia_aviso_semanal ?? 3,
          horario_aviso_semanal: data.horario_aviso_semanal ? data.horario_aviso_semanal.slice(0, 5) : '09:00',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        dia_aviso_semanal: form.aviso_semanal_ativo ? form.dia_aviso_semanal : null,
        horario_aviso_semanal: form.aviso_semanal_ativo ? form.horario_aviso_semanal : null,
  ...
  return (
    <div className="space-y-4 max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Card className="space-y-4">
          <p className="text-xs uppercase tracking-wide text-text-muted">Lembrete push semanal</p>
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={form.aviso_semanal_ativo}
              onChange={(e) => set('aviso_semanal_ativo', e.target.checked)}
              className="h-4 w-4 accent-[#caff3b]"
            />
            Receber notificação push lembrando de enviar a lista no WhatsApp
          </label>
          {form.aviso_semanal_ativo && (
            <div className="grid grid-cols-2 gap-3">
              <Select
                label="Dia da semana"
                value={form.dia_aviso_semanal}
                onChange={(e) => set('dia_aviso_semanal', Number(e.target.value))}
              >
                {DIAS.map((d) => (
                  <option key={d} value={d} className="capitalize">
                    {diaSemanaLabel(d)}
              ...
              <Input
                ...
                type="time"
                required
                value={form.horario_aviso_semanal}
                onChange={(e) => set('horario_aviso_semanal', e.target.value)}
              />
            </div>
          )}
          <p className="text-xs text-text-muted normal-case">
            Todo dia e horário escolhidos, o FOOTER envia uma notificação push para o seu celular lembrando de
            enviar o link da lista no grupo do WhatsApp. Ative as notificações push em Notificações para receber.
          </p>
        </Card>
